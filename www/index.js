var user_name = sessionStorage.getItem('user_name') || ""; // 从 sessionStorage 获取用户名
// 页面加载时弹出输入框
if (!user_name || user_name.trim() === '') {
    user_name = prompt("请输入您的用户名:");
    if (user_name) {
        console.log("用户名已设置为:", user_name);
        sessionStorage.setItem('user_name', user_name); // 将用户名保存到 sessionStorage
    } else {
        console.log("未输入用户名");
    }
};
...
const wsUrl = `ws://localhost:8081?username=${encodeURIComponent(user_name)}`;
...
function addChatItem(uid, message) {
  ...
  chatItem.innerHTML = `
    <div class="chat-item_user">${uid}${uid === me.id ? '（我）': ''} :</div>
...
signalingServer.onmessage = ({ data: responseStr }) => {
  ...
  if (type === '1000') {
    addChatItem('System', data.message);
    return;
  }
  if (type === '1001') {
    me.id = data.id;
...
toggleButton.addEventListener('click', () => {
    right.classList.toggle('sidebar-expanded');
});
window.addEventListener('resize', () => {
    if (window.innerWidth > 768) {
        right.classList.remove('sidebar-expanded');
    } else {
        right.classList.add('sidebar-expanded');
    }
});
