...
// 消息类型定义
const SEND_TYPE_SYSTEM = '1000'; // 系统消息
...
server.on('connection', (socket, request) => {
  ...
  const userName = urlParams.get('username'); // 获取用户名
  const currentId = service.registerUser(ip, socket, userName);
  // 向客户端发送自己的id
  ...
  service.getUserList(ip).forEach(user => {
    ...
    // 向所有用户发送新用户加入的通知
    send(user.socket, SEND_TYPE_SYSTEM, { message: `${userName} has joined the room.` });
});
